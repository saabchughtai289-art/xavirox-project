...
// 1. DATABASE CONNECTION (STABLE SRV)
...
app.post('/login', async (req, res) => {
    ...
    if (user && await bcrypt.compare(req.body.password, user.password)) {
        ...
        res.send("<script>alert('Invalid Access'); window.location='/login';</script>");
    }
});

// 5. THE MASTER DASHBOARD (ALL FEATURES + MOBILE FIX)
                ...
                <div class="post-header">
                    ...
                    <div class="post-meta">
                        <span class="author-name">
                            @${p.author} ${p.hasBlueTick ? '<i class="fas fa-check-circle" style="color:#ff007f;"></i>' : ''}
                        </span>
            ...
            <head>
                ...
                <style>
                    ...
                    * { box-sizing: border-box; }
                    ...
                    .post-card:hover { border-color: var(--pink); box-shadow: 0 0 15px rgba(255,0,127,0.1); }
                    ...
                    .author-name { color: var(--pink); font-weight: bold; }
                    ...
                    @media (max-width: 900px) {
                        .main-layout { flex-direction: column; align-items: center; padding-top: 80px; }
                        .sidebar, .feed { width: 100%; position: relative; top: 0; }
                        .sidebar { order: 1; } .feed { order: 2; }
                    }
                </style>
            </head>
            <body>
                <nav class="navbar">
                    ...
                    <a href="/logout" style="color:var(--pink); text-decoration:none; font-weight:bold;">DISCONNECT</a>
                </nav>
                    ...
                    <div class="sidebar">
                        <div class="glass-card" style="text-align:center;">
                            ...
                            <form action="/update-portfolio" method="POST" style="margin-top:15px;">
                                ...
                                <button class="transmit-btn" style="padding:8px; font-size:10px;">UPDATE LINK</button>
                            </form>
                        </div>

                        <div class="glass-card">
                            <h4 style="color:var(--pink); margin-top:0;">COMMUNITIES</h4>
                            <form action="/create-comm" method="POST">
                                <input name="name" placeholder="Name..." style="width:100%; padding:8px; background:none; border:1px solid rgba(255,255,255,0.1); color:#fff; border-radius:8px;">
                                <button class="transmit-btn" style="padding:8px; font-size:10px;">FOUND</button>
                            </form>
                            <div style="margin-top:10px; font-size:12px; color:rgba(255,255,255,0.6);">
                                ${comms.map(c => `<div># ${c.name}</div>`).join('')}
                            </div>
                        </div>
                        ...
                        <div class="glass-card">
                            ...
                            <div style="font-size:10px; opacity:0.7;">
                                ${user.savedPosts.map(sp => `• ${sp.content.substring(0,25)}...<br>`).join('')}
                            </div>
                        </div>

                        <div class="glass-card">
                            <h4 style="color:var(--pink); margin-top:0;">FEEDBACK</h4>
                            <textarea id="fb" placeholder="System report..." style="height:60px; font-size:12px;"></textarea>
                            <button onclick="sendFB()" class="transmit-btn" style="padding:8px; font-size:10px;">SEND</button>
                        </div>
                    </div>
                ...
                <script>
                    ...
                    async function savePost(id) {
                        ...
                        if(res.ok) { alert("Signal Saved!"); location.reload(); }
                    }

                    async function sendFB() {
                        const msg = document.getElementById('fb').value;
...
app.post('/api/feedback', isAuth, async (req, res) => {
...
app.post('/create-comm', isAuth, async (req, res) => {
    try {
        await new Community({ name: req.body.name, creator: req.session.user.username }).save();
        res.redirect('/dashboard');
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 XAVIROX CORE LIVE ON ${PORT}`));